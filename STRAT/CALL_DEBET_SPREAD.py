...
async def call_debet_spread_strat(ib, vix_df, input_data, yahoo_stock):
    strategy = 'CALL DEBET SPREAD'
    ...
    call_30_delta_position = current_input_data.Short.values[0]

    # ДАТЫ ЭКСПИРАЦИИ
    limit_date_min = datetime.datetime.now() + relativedelta(days=+40)
    ...
    # УСЛОВИЯ ВХОДА
    if current_price > sma_100 and sma_20 > sma_100 and 30 < rsi < 70:
        if df_iv['IV_percentile'].iloc[-1] < 50:
            ...
            if vix_signal <= 2:
                ...
                delta_30_strike = needed_strike_delta(df_chains, 0.3, 'long')
                call_30_delta = df_chains[df_chains['Strike'] == delta_30_strike].reset_index(drop=True).iloc[0]

                contract_to_buy = call_30_delta['contract']
                contract_to_sell = atm_call['contract']
                ...
                if check_to_open(contract_to_buy, contract_to_sell, strategy, tick):
                    ...
                    order_sell = MarketOrder("Sell", call_30_delta_position)
