...
async def spy_put_credit_spread_strat(ib):
    ...
    while True:  # datetime.datetime.strptime('2022-02-01', "%Y-%m-%d")
        ...
        tick = current_input_data.Stock
        atm_put_position = current_input_data.Long
        atm_put_1_abobe_position = current_input_data.Short

        stock_price_df_native = yf.download(tick)
        vix_df = yf.download('^VIX')

        stock_price_df_native['RSI'] = pta.rsi(stock_price_df_native['Close'])
        stock_price_df_native['SMA_20'] = stock_price_df_native['Close'].rolling(window=20).mean()
        stock_price_df_native['SMA_100'] = stock_price_df_native['Close'].rolling(window=100).mean()
        ...
        sma_20 = stock_price_df_native['SMA_20'].iloc[-1]
        sma_100 = stock_price_df_native['SMA_100'].iloc[-1]
        rsi = stock_price_df_native['RSI'].iloc[-1]
            ...
            if 30 < rsi < 70:
                if df_iv['IV_percentile'].iloc[-1] > 50:
                    ...
                    if vix_signal <= 2:
                        ...
                        atm_1_above_strike = df_chains_local[df_chains_local['Strike'] > atm_strike].reset_index(drop=True).iloc[0]
                        ...
                        if check_to_open(theoretical_position, strategy, tick):
                            ...
                            exp_contract_to_sell = datetime.datetime.strptime(contract_to_sell.lastTradeDateOrContractMonth, "%Y%m%d")
                            ...
                            logg_df = logging_open(trade_sell, contract_to_sell, order_sell, strategy, hard_position, ib, condition, time_to_exp_sell)
                            print('end while')
