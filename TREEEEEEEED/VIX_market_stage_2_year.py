...
def market_stage_vix_2_year(df):
    ticker = '^VIX'
    start_vix_price = dt.datetime.now() - relativedelta(years=2)
    ...
    stage = Regimes['Regime'].iloc[-1]

    Regimes_plot = Regimes[Regimes['Regime'] == stage]

    # fig1 = px.scatter(Regimes_plot, x="Date", y="close", color="Regime", )
    #
    # fig1.show()

    percentile = round(stats.percentileofscore(Regimes_plot['close'], Regimes_plot['close'].iloc[-1]), 2)

    return stage, percentile
